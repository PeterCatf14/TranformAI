...
function initOnboarding() {
    ...
    document.getElementById('btn-next').addEventListener('click', () => {
        ...
        if (step === 1) {
            ...
            data.weight = parseFloat(document.getElementById('input-weight').value);
...
function completeOnboarding(data) {
    state.profile = data;
    state.weeklyPlan = generateWeeklyPlan(data.goal);
...
function showMainApp() {
    ...
    if (!state.weeklyPlan) state.weeklyPlan = generateWeeklyPlan(state.profile.goal);
    updateDashboard();
    renderWeeklyPlan();
    renderNutrition();
    renderProfile();
...
function renderProfile() {
    ...
    if (!p) return;
    document.getElementById('user-name-display').textContent = GOAL_LABELS[p.goal] ? '💪' : '';
    document.getElementById('profile-name').textContent = 'Mi Perfil';
    ...
    document.getElementById('profile-weight').textContent = p.weight + ' kg';
