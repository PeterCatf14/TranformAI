...
const HOME_EXERCISES = {
    ...
    back: [
        { name: 'Dominadas en el suelo (Floor Pulls)', sets: 4, reps: 15, weight: 0 },
        { name: 'Remo invertido en mesa', sets: 4, reps: 10, weight: 0 },
        { name: 'Supermans', sets: 3, reps: 15, weight: 0 },
        { name: 'Caminata de oruga (Inchworms)', sets: 3, reps: 12, weight: 0 }
    ],
    legs: [
    ...
    shoulders: [
        ...
        { name: 'Plancha con toques al hombro', sets: 3, reps: 20, weight: 0 },
    ...
    arms: [
        ...
        { name: 'Plancha dinámica (Plank to Pushup)', sets: 3, reps: 12, weight: 0 },
...
function showMainApp() {
    ...
    if (!state.profile) {
        // showOnboarding doesn't exist - manipulate DOM directly
        document.getElementById('onboarding-screen').classList.add('active');
        return;
    }
    ...
    setElementText('user-name-display', state.profile.name || 'Usuario');
    ...
    if (!state.weeklyPlan) {
        state.weeklyPlan = generateWeeklyPlan(state.profile.goal, state.profile.sex, state.profile.workoutMode || 'gym');
        Store.set('weeklyPlan', state.weeklyPlan);
    }
    ...
    try {
        ...
        console.error('showMainApp init error:', e);
    }
}
...
function updateDashboard() {
    ...
    try {
        ...
        setElementText('ring-burn-value', burnedToday);
        setElementText('ring-work-value', workoutsThisWeek);
        setElementText('ring-steps-value', stepCount.toLocaleString());
        setElementText('ring-water-dash-value', waterLvl);
        setElementText('ring-consumed-value', consumed.cal);
        setElementText('dash-water-val', waterLvl + ' ml');
        ...
        const pb = document.getElementById('dash-protein-bar'); if(pb) pb.style.width = pPct + '%';
        const cb = document.getElementById('dash-carbs-bar'); if(cb) cb.style.width = cPct + '%';
        const fb = document.getElementById('dash-fats-bar'); if(fb) fb.style.width = fPct + '%';
        
        setElementText('dash-protein-val', `${consumed.protein}g / ${macros.protein}g`);
        setElementText('dash-carbs-val', `${consumed.carbs}g / ${macros.carbs}g`);
        setElementText('dash-fats-val', `${consumed.fats}g / ${macros.fats}g`);

        const todayIdx = (new Date().getDay() + 6) % 7;
        const todayW = (state.weeklyPlan && state.weeklyPlan[todayIdx]) ? state.weeklyPlan[todayIdx] : null;
        setElementText('today-workout-name', todayW ? todayW.name : 'Descanso');
        ...
        if (previewEl) {
            ...
            if (todayW && !todayW.rest && todayW.exercises) {
                todayW.exercises.slice(0, 4).forEach(ex => {
                    previewEl.innerHTML += `<div class="preview-exercise"><span class="preview-exercise-name">${ex.name}</span><span class="preview-exercise-detail">${ex.sets}×${ex.reps}</span></div>`;
                });
            }
        }
        const startBtn = document.getElementById('btn-start-today-workout');
        if (startBtn) {
            startBtn.onclick = () => { if (todayW && !todayW.rest) { switchTab('workout'); startWorkout(todayIdx); } };
            startBtn.style.display = (todayW && !todayW.rest) ? '' : 'none';
        }
...
function renderWeeklyPlan() {
    ...
    try {
        ...
        if (!cont) return;
        cont.innerHTML = '';
        setElementText('workout-week-badge', `Semana ${state.weekNum}`);
        const todayIdx = (new Date().getDay() + 6) % 7;
        ...
        console.error("Weekly Plan Err:", e);
    }
}
...
function startWorkout(dayIndex) {
    ...
    if (cancelBtn) {
        cancelBtn.onclick = () => {
            if (confirm('¿Cancelar entrenamiento sin guardar?')) {
                endWorkoutView();
            }
        };
    }
...
function renderNutrition() {
    ...
    try {
        renderNutritionWater();
        const macros = calcMacros(state.profile);
        setElementText('cal-target-display', macros.calories + ' kcal');
        setElementText('macro-protein-target', macros.protein + 'g');
        setElementText('macro-carbs-target', macros.carbs + 'g');
        setElementText('macro-fats-target', macros.fats + 'g');
